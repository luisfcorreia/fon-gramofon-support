...
complete_setup() {
    ...
    # Get initial status
    echo -e "${BLUE}1. Initial Status${NC}"
    get_status
    echo ""
    
    # Scan networks
    echo -e "${BLUE}2. Scanning Networks${NC}"
    scan_networks
    echo ""
    
    # Configure WiFi
    echo -e "${BLUE}3. Configuring WiFi${NC}"
    configure_wifi "$ssid" "$password" "$device_name"
    echo ""
    
    # Get final status
    echo -e "${BLUE}4. Final Status${NC}"
    get_status
    echo ""
    
    echo -e "${GREEN}=== Configuration Complete ===${NC}"
    echo -e "Note: Device may reboot. Wait 30-60 seconds before reconnecting."
}
